...
export default function LoginPage() {
  ...
  function handleLogin(e: FormEvent) {
    ...
    let users: Array<{ username: string; email: string }> = [];
    try {
      users = JSON.parse(localStorage.getItem("qlab_users") || "[]");
    } catch {
      users = [];
    }

    const found = users.find(
      (u) => u.username === loginUsername.trim()
    );

    if (!found) {
      if (users.length > 0) {
        setLoginErrors({ general: "用户名不存在" });
        return;
      }
    }
    ...
    <div className="mx-auto max-w-lg px-6 py-20">
      ...
      <div className="mt-8 rounded-lg border border-accent-yellow/20 bg-accent-yellow/5 px-5 py-4">
        <div className="flex items-start gap-2">
          ...
          <div>
            <p className="text-xs font-medium text-foreground/80">说明</p>
            <p className="mt-1 text-xs leading-relaxed text-muted">
              本站为个人量化学习研究记录平台。注册账号用于记录个人学习进度。
            </p>
          </div>
        </div>
      </div>
