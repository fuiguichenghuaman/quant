...
const PYTHON_KEYWORDS = new Set([
  ...
  "False", "try", "except", "finally", "with", "as", "lambda",
  "yield", "raise", "pass", "break", "continue", "global", "nonlocal",
  "assert", "del", "print",
]);

const PYTHON_BUILTINS = new Set([
  "np", "plt", "Path", "print", "len", "range", "enumerate", "zip",
  "map", "filter", "int", "float", "str", "list", "dict", "tuple",
  "set", "bool", "type", "isinstance", "format", "open", "super",
]);
...
function highlightPythonLine(line: string): React.ReactNode {
  ...
  while (remaining.length > 0) {
    ...
    if (remaining.startsWith('"""') || remaining.startsWith("'''")) {
      ...
      if (endIdx !== -1) {
        parts.push(
          <span key={partKey++} className="text-amber-300">
            {remaining.slice(0, endIdx + 3)}
          </span>
        );
    ...
    if (numMatch) {
      parts.push(
        <span key={partKey++} className="text-purple-300">
          {numMatch[1]}
        </span>
      );
    ...
    if (wordMatch) {
      ...
      if (PYTHON_KEYWORDS.has(word)) {
        parts.push(
          <span key={partKey++} className="text-blue-300">
            {word}
          </span>
        );
      } else if (PYTHON_BUILTINS.has(word)) {
        parts.push(
          <span key={partKey++} className="text-cyan-300">
            {word}
          </span>
        );
      } else {
    ...
    if (remaining.startsWith("@")) {
      ...
      if (decoMatch) {
        parts.push(
          <span key={partKey++} className="text-yellow-300">
            {decoMatch[1]}
          </span>
        );
    ...
    <div className="mx-auto max-w-4xl px-6 py-12">
      ...
      {explainedPyContent && (
        <section className="mb-12">
          ...
          <div className="overflow-x-auto rounded-xl bg-code-bg p-5">
            <pre className="text-sm leading-relaxed font-mono">
              <code className="text-white/90">
                {highlightPython(explainedPyContent)}
              </code>
            </pre>
          </div>
        </section>
      ...
      <section className="mt-16 border-t border-border pt-6">
        <p className="text-center text-xs leading-relaxed text-muted">
          本页面内容仅为个人学习记录，不构成投资建议。不荐股、不承诺收益、不带单。
          <br />
          代码和数据仅用于教学演示，不代表真实交易结果。
        </p>
      </section>
    </div>
  );
